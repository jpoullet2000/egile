...
async def test_stock_updates_comprehensive():
    ...
    try:
        ...
        for i, (user_message, expected_operation, product_name) in enumerate(test_cases, 1):
            ...
            try:
                ...
                if not success:
                    error_msg = action_result.get("error", "Unknown error") if action_result else "No action result"
                ...
                if missing_elements:
                    ...
                    print("   ✅ Enhanced confirmation message contains all required elements")
                
                # Verify intent analysis detected correct operation
                parameters = intent.get("parameters", {})
                detected_operation = parameters.get("operation", "unknown")
                
                if expected_operation == "increment" and detected_operation in ["add", "increment"]:
                    print(f"   ✅ Operation correctly detected: {detected_operation}")
                elif expected_operation == "set" and detected_operation == "set":
                    print(f"   ✅ Operation correctly detected: {detected_operation}")
                else:
                    print(f"   ⚠️  Operation detection issue - Expected: {expected_operation}, Got: {detected_operation}")
